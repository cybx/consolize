...
Write-Host ""
Write-Host "To read a reply, wait for it: consolize is a GUI subsystem binary (so"
Write-Host "that being the shell never flashes a console window), and PowerShell"
Write-Host "does not wait for those, which scrambles the output of a plain call."
Write-Host "    Start-Process '$installDir\consolize.exe' -ArgumentList 'send status' -NoNewWindow -Wait"
Write-Host "    Start-Process '$installDir\consolize.exe' -ArgumentList 'send quit'   -NoNewWindow -Wait"
